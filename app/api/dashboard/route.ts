...
import { clientLoans, osintResults, fraudAnalysis } from "@/server/db/schema";
  ...
  return withAuth(async (user) => {
    try {
      ...
      const osint = clientIds.length
        ? await db.select().from(osintResults).where(inArray(osintResults.clientId, clientIds))
        : [];

      const fraud = clientIds.length
        ? await db.select().from(fraudAnalysis).where(inArray(fraudAnalysis.clientId, clientIds))
        : [];

      const totalClients = scopedClients.length;
      const totalBalance = loans.reduce((sum: number, l: LoanRef) => sum + n(l.balance), 0);
      const totalOverdue = loans.reduce((sum: number, l: LoanRef) => sum + n(l.overdue), 0);

      const avgRisk =
        (osint as OsintRef[]).reduce((sum: number, o) => sum + n(o.confidenceScore), 0) / (osint.length || 1);
