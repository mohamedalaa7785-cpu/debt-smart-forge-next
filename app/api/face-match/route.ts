...
export async function POST(req: NextRequest) {
  return withAuth(async (user) => {
    try {
      ...
      const parsed = FaceMatchBodySchema.safeParse(await req.json().catch(() => ({})));
      if (!parsed.success) {
      ...
      const result = await compareFaceSimilarity(parsed.data.imageBase64A, parsed.data.imageBase64B);
