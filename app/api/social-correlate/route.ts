...
export async function POST(req: Request) {
  try {
    console.log("[social-correlate] request received");
    await requireUser();
    ...
    if (duplicate.length && !force) return NextResponse.json({ success: true, data: { queued: false, reason: "duplicate_recent_search" }, error: null });
    ...
    console.error("[social-correlate] error", error);
...
export async function GET(req: Request) {
  try {
    ...
    console.error("[social-correlate] GET error", error);
    return NextResponse.json({ success: false, data: [], error: error instanceof Error ? error.message : "Unknown error" }, { status: 500 });
  }
}
