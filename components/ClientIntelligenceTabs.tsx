...
export default function ClientIntelligenceTabs({ clientId, clientName, primaryPhone }: { clientId: string; clientName: string; primaryPhone: string }) {
  const tabs = ["Overview", "OSINT", "Social Media", "Phone Intelligence", "Risk Analysis", "Timeline"] as const;
  ...
  return (
    <div className="bg-slate-900 text-slate-100 rounded-2xl p-4 md:p-6 border border-slate-700">
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mt-4 space-y-4">
        {(active === "Overview" || active === "Phone Intelligence") && <div className="grid md:grid-cols-3 gap-3">
        ...
        {phoneResult && (active === "Overview" || active === "Phone Intelligence") && <div className="p-4 rounded-xl bg-slate-800 border border-slate-700">
        ...
        {active === "Timeline" && <div className="text-sm text-slate-300">Phone search history ({history.length}) displayed in investigations context.</div>}
      </motion.div>
    </div>
  );
}
